...
The cleaned dataset consisted of `r nrow(wnba_complete)` records and `r ncol(wnba_complete)` variables, out of which four percentage variables which also had some missing values.
...
The average R-squared across all models was `r mean(regsubsets_models2$Rsquared, na.rm = T)`.
...
Judging by histograms and Q-Q plots the distributions of residuals for the most part conform to the normal distributions with slight deviations at the tails,
...
All models suffered from heteroscedasticity, with a lower error variance at the smaller predicted values, which was more pronounced in the models fitted to the untransformed data.
...
No influential values were observed as judged by cook's distance.
...
A high number of models fitted to the transformed datasets lacked R-square estimation.
...
The average R-squared across all models was `r mean(lasso_models$Rsquared, na.rm = T)`.
...
```{r}
# Extract 10 best models
best10_tree <- tree_models2 %>% 
  select(-fit) %>% 
  slice(1:10)
...
The 10 best tree models explained between 
`r min(best10_tree$Rsquared)`
`r max(best10_tree$Rsquared)` proportion of the total variance,
with complexity paramter being between
`r min(best10_tree$cp)` and
`r max(best10_tree$cp)` predictors.

Across all models the average R-squared was equal to `r mean(tree_models2$Rsquared)`.


Interestingly, all 10 best models were fitted to the full and untransformed dataset.
...
The average R-squared across all Random Forest models was `r mean(forest_fitted$Rsquared)`.
...
The average R-squared across all gradient-boosted tree models was `r mean(boosted_models2$Rsquared)`.
...
The gradient-boosting tree models performed better than the standard tree models, but worse in comparison to both linear and random forest models. 
In terms of the best predictors, boosted trees followed a similar pattern to Random Forest models, revealing field goals, games, games started, personal fouls and two-points goals as the key independent variables. 

However, instead of Points as it was in the case of random forest models, the sixth crucial predictor was number of assists. 
Also in contrast to random forest models, gradient-boosting tree models were especially well fitted to the reduced and log-transformed dataset.
