...
best_boosted$fit[[1]]
partial(best_boosted$fit[[1]], pred.var =  c("Games", "GS"), train = model_datasets$x_train[[1]])
...
best_boosted2 <- best_boosted %>% 
  left_join(model_datasets) %>% 
  mutate(
    plots_Games = future_map2(fit, x_train, ~partial(.x, train = .y, pred.var = "Games")),
    plots_GS = future_map2(fit, x_train, ~partial(.x, train = .y, pred.var = "GS")),
    plots_FG = future_map2(fit, x_train, ~partial(.x, train = .y, pred.var = "FG")),
    plots_Two_P = future_map2(fit, x_train, ~partial(.x, train = .y, pred.var = "Two_P")),
    plots_PF = future_map2(fit, x_train, ~partial(.x, train = .y, pred.var = "PF"))
  ) %>% 
  select(dataset, starts_with("plots")) %>% 
  pivot_longer(-dataset, names_to = "predictor_name", values_to = "partial_predictions") 
...
best_boosted2$partial_predictions[[1]]
