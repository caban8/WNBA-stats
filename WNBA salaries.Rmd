...
wnba_complete %>% 
  mutate(year = as.double(as.character(year)) + 1) %>%  # Move salaries back to their true date
...
salaries_stats <- wnba_combined2 %>% 
  ...
  ungroup() %>% 
  mutate(year = as.double(as.character(year)) + 1)
...
Starting with 2019, a quarter of the players were earning more than $150,000 after adjusting for inflation, which was the maximum salary before that year.

In 2020, the spread of the salaries appeared to stabilize.
spread of the salaries seemed to stabilize.   
...
# Funkcja do zliczania korekty year 
...
None of the continuous predictors shows a linear relationship with salary.
