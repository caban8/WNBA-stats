...
# Load all libraries
pacman::p_load(xgboost, DT, readxl,leaps,furrr, rstatix, broom, bestglm, glmnet, leaps, car,  pROC, caret, tree, rpart, randomForest, rpart.plot, doParallel, tidyverse)
...
# Fit lm using cross-validations
...
subset_lm <- function(regsubset, id, data) {
  ...
  fit_lm <- train(form, data = data, method = "lm", trControl = train_control)
  
  fit_lm$results %>% 
    select(RMSE, Rsquared, MAE) %>% 
    as_tibble() %>% 
    mutate(fit_lm = list(fit_lm$finalModel))
}
...
lm_models <- regsubsets_models2 %>% 
  group_by(dataset) %>% 
  arrange(RMSE) %>% 
  slice(1) %>% 
  select(dataset, fit_lm) 


regsubsets_models2 %>% 
  select(dataset, id, method, RMSE, MAE, Rsquared) %>% 
  arrange(RMSE) %>% 
  kable2() 
...
lm_models <- lm_models %>% 
  mutate(
    coefs = map(fit_lm, ~lm.beta::lm.beta(.) %>% tidy()),
    augmented = map(fit_lm, augment),
    vif = map(fit_lm, vif)
    )
...
# Extract function for tree model
cross_lasso <- function(x_data, y_data) {
  
  # Create search grid for lambda parameter
  ...
  model$results %>% 
    select(lambda, RMSE, Rsquared, MAE) %>% 
    as_tibble() %>% 
    mutate(fit_lasso = list(model$finalModel))
...
lasso_models <- model_datasets %>% 
  ...
  mutate(
    fit_lasso = map2(x_train, y_train, cross_lasso),
    ) %>% 
  unnest(fit_lasso) 
...
best_lasso <- lasso_models %>% 
  ...
  arrange(RMSE) %>% 
  slice(1) %>% 
  select(dataset, lambda, fit_lasso) %>% 
  ungroup()


lasso_models %>% 
  select(-c(fit_lasso, x_train, y_train)) %>% 
  arrange(RMSE)
...
best_lasso <- best_lasso %>% 
  mutate(
    coefs = map2(fit_lasso, lambda, ~coef(.x, s = .y)),
...
best_lasso_reduced <- best_lasso %>% 
  ...
  ungroup() %>% 
  mutate(
    fit_lm = map2(formula, train_dummy, ~lm(.x, .y))
  ) %>% 
  select(dataset, fit_lm) 
...
```{r}
# Set multisession for parallel computing for specific chunks of code
plan(multisession, workers = length(availableWorkers()) - 4)

# Train very long trees
tree_models <- model_datasets %>% 
  select(dataset, train) %>% 
  mutate(
    tree_fit = map(
      train, 
      ~tree(general_formula, .x, control = tree.control(
        mindev = 0.0001,
        nobs = nrow(.x)
        )))
  )
...
# Extract function for tree model
cross_tree <- function(data) {
  
  # Create search grid for cp parameter
  cp <- data.frame(cp = seq(0.001, 0.5, 0.005) )
  ...
  model$results %>% 
    select(cp, RMSE, Rsquared, MAE) %>% 
    as_tibble() %>% 
    mutate(fit_tree = list(model$finalModel))
...
tree_models <- model_datasets %>% 
  select(dataset, train) %>% 
  mutate(fit_tree = future_map(train, cross_tree))
...
best_tree %>% 
  mutate(plot = map(fit_tree, rpart.plot))
...
```{r}
# Extract function for randomForest fit
cross_forest <- function(data) {
  
  # Create search grid for mtry parameter
  mtry <- data.frame(mtry = seq(2,ncol(data), 2)) 
  ...
  model$results %>% 
    ...
    as_tibble() %>% 
    mutate(fit_forest = list(model$finalModel))
}  
...
forest_models <- model_datasets %>% 
  select(dataset, train) %>% 
  mutate(fit_forest = map(train, cross_forest))
...
forest_models %>% 
  unnest(fit_forest) %>% 
  select(dataset, mtry, RMSE, Rsquared, MAE) %>% 
  arrange(RMSE)
...
best_forest2 <- best_forest %>% 
  mutate(
    importance_purity = map(fit_forest, ~importance(.) %>% as.data.frame() %>% rownames_to_column())
    )
...
best_forest2 %>% 
  ...
  unnest() %>% 
  arrange(dataset, desc(IncNodePurity)) 
...
randomForest::partialPlot(
  best_forest$fit_forest[[2]], 
  x.var = Games, 
  pred.data = model_datasets$train[[2]]
  )
...
# Extract function for xgb boosting fit
cross_boost <- function(data) {
  ...
  model$results %>% 
    select(nrounds, max_depth, eta, RMSE, Rsquared, MAE) %>% 
    as_tibble() %>% 
    mutate(fit_boost = list(model$finalModel))
}  
...
boosted_models <- model_datasets %>% 
  select(dataset, train) %>% 
  mutate(
    fit_boost = map(train, cross_boost)
  )
...
best_boosted %>% 
  rename(fit_forest = fit_boost) %>% 
  add_row(best_forest) %>% 
  left_join(model_datasets) %>% 
  mutate(
    rmse = map2(fit_forest, test, predict)
  )
