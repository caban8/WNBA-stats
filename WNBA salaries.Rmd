...
forest_models2 %>% 
  save(file = "Random Forest models.Rdata")
...
load("Random Forest models.Rdata")
...
Overall, models with a higher number of `mtry` were associated with a higher R-squared. 
A more pronounced difference was observered for number of trees used in random forest fitting. 
...
forest_performance <- forest_fitted %>% 
  ...
  mutate(
    ...
    transformed = str_detect(dataset, "log")
    ) %>% 
  ungroup() %>% 
...
for (i in 1:nrow(forest_performance)) {
  ...
  plot_list[[i]] <-  forest_performance$data[[i]] %>% 
