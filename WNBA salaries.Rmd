...
# Load all libraries
pacman::p_load(pdp, modelr, xgboost, DT, readxl,leaps,furrr, rstatix, broom, bestglm, glmnet, leaps, car,  pROC, caret, tree, rpart, randomForest, rpart.plot, doParallel, tidyverse)
...
pivoted_Onsalary %>% 
  ...
  geom_hex() +
  geom_smooth(se = F,  alpha = 2/3) +
  scale_fill_viridis_c(option = "A", direction = -1) +
  facet_wrap(~name, scales = "free", ncol = 5)

```

Higher number of goals, regardless of its specific type, were generally associated with higher salaries, although in some cases - like 2-points goals and field goals and free throws - up to a given level. 
A similar trend was observed for 
In some cases there was a reverse at the higher values of a predictor with a small drop in salary, but those were solely to few outlier values and as such can be disregarded. 

Some of the relationships were linear in approximations (3-points goals and number of assits), but the majority were non-linear and therefore more complex. 

, when analyzed using simple bivariate comparisons, displayed either no or non-linear and complex relationship with salary.

This suggests that the tree models should perform better.
...
However, one performance statistis stood out - 3-point goals, for the part, was either weakly or moderately correlated with t he remaining predictors and with one (Offensive Rebounds), it was even negatively associated. 

Interestingly, the goal metrics, when expressed in proportion of successes to attempts, had mostly weak associations and sometimes negative with the rest of the predictors.
This might be explained by their tendency to distort the true performance of the players who made only few attempts in total.
...
Five of the models fitted to the full and transformed dataset lacked R-squared estimation.
Some models fitted to the reduced transformed dataset resulted in very small R-squared values.
...
The reported missing values in resampled performance measures seems to be associated with the models fitted to the dataset with the log-transformed outcome that relied on a small learning rate and a small number of boosting iterations.
Given this small size of the two tuning parameters and that the outcome was logarithmized, the models probably did not have enough iterations to converge properly, i.e. they failed to learn meaningful patterns.
...
The best 10 boosted tree models, as revealed by cross-validation, explained between 
`r min(boosted_best10$Rsquared)`
`r max(boosted_best10$Rsquared)` proportion of the total variance,
with mtry being between
`r min(best_forest10$mtry)` and
`r max(best_forest10$mtry)` predictors.
...
In contrast, the more the Games and Personal Fouls, the smaller the salary.
Salary strongly dropped with around 10 Personal Fouls and remained at a similar level until another drop with more than 50 fouls.
In the case of number of Games, salary started consistently dropping after above more than 10 games.
